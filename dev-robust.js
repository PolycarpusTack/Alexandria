...
/**
 * Robust development server starter that works reliably on Windows
 * No external dependencies - uses built-in Node.js child_process
 */
...
// Start server first
const serverProcess = spawnRobust('pnpm', ['run', 'dev:server'], 'SERVER', 'server');

// Wait for server to initialize, then start client
setTimeout(() => {
  const clientProcess = spawnRobust('pnpm', ['run', 'dev:client'], 'CLIENT', 'client');
