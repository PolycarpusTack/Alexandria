...
// Define Joi-like validation interface for fallback
interface JoiLikeValidator {
  validate: (data: unknown) => { error: null | Error; value: unknown };
}

interface JoiLikeStringValidator {
  valid: (...args: unknown[]) => { default: (val: unknown) => unknown };
}

interface JoiLikeNumberValidator {
  integer: () => { min: () => { max: () => { default: (val: unknown) => unknown } } };
}

interface JoiLike {
  object: (schema: unknown) => JoiLikeValidator;
  string: () => JoiLikeStringValidator;
  number: () => JoiLikeNumberValidator;
}
...
try {
  ...
  Joi = {
    object: (schema: unknown) => ({
      validate: (data: unknown) => ({ error: null, value: data })
    }),
    string: () => ({ valid: (...args: unknown[]) => ({ default: (val: unknown) => val }) }),
    number: () => ({
      integer: () => ({ min: () => ({ max: () => ({ default: (val: unknown) => val }) }) })
    })
  };
}
...
import { getNetworkMetrics } from '../utils/network-monitor';
...
router.get('/stats/summary', async (req: Request, res: Response) => {
  try {
    ...
    const totalRequests = requestLogs.filter(
      (log) => log.source === 'api' && log.context?.type === 'request'
    ).length;

    const totalErrors = requestLogs.filter(
      (log) => log.level === 'error' || (log.context?.statusCode && log.context.statusCode >= 400)
    ).length;
  ...
  try {
    ...
    try {
      ...
      logs.forEach((log) => {
        ...
        const bucketTime =
          Math.floor((logTime - startTime.getTime()) / intervalMs) * intervalMs +
          startTime.getTime();
