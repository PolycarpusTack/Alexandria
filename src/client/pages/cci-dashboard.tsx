    ...
    <div className="grid grid-cols-[2fr_1fr] gap-5">
      <div className="space-y-5">
        <Card className={`p-5 shadow-sm ${isDark ? 'bg-gray-800 border-gray-700' : ''}`}>
          ...
          <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>No new announcements.</p>
        </Card>
        
        <Card className={`p-5 shadow-sm ${isDark ? 'bg-gray-800 border-gray-700' : ''}`}>
          <h2 className="text-lg font-medium text-[var(--primary-color)] mb-3">Plugins Overview</h2>
          <ul className={`list-none p-0 m-0 divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
            {plugins.map(plugin => (
              <li key={plugin.id} className="flex items-center py-3">
                <i className="fa-solid fa-microchip mr-2 text-[var(--primary-color)]"></i>
                {plugin.name}
                <span className={`h-2 w-2 bg-${plugin.status}-500 rounded-full inline-block mx-2`}></span>
          ...
          <ul className={`list-none p-0 m-0 divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
            {activity.map((item, index) => (
              <li key={index} className="flex items-center py-3">
                ...
                <span className="text-sm"><strong>{item.time}</strong> — {item.message}</span>
              </li>
            ))}
          </ul>
