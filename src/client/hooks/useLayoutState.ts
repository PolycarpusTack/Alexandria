...
export function useLayoutState(
...
): [LayoutState, LayoutActions & { error: any; hasError: boolean; clearError: () => void }] {
    ...
    (query: string) => {
      try {
        ...
        logger.debug('Search query changed', {
          searchQuery: query.substring(0, 50) + (query.length > 50 ? '...' : '')
        });
      } catch (error) {
  ...
  const setQuickAccessOpen = useCallback(
    (open: boolean) => {
      try {
        if (typeof open !== 'boolean') {
          throw new UIOperationError(
