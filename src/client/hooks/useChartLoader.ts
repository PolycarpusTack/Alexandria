...
export function useChartLoader() {
  ...
  const createChart = useCallback((config: ChartConfig, chartData?: ChartData) => {
    try {
      if (!isChartLibraryLoaded || !(window as any).Chart) {
        throw new UIOperationError(
          'chart-library-not-ready',
          'Chart.js library not loaded yet',
          { 
  ...
  const destroyChart = useCallback((canvasId: string) => {
