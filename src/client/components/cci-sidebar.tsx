...
function SidebarItem({ icon, label, to, status, children }: SidebarItemProps) {
  ...
  return (
    ...
    >
      <i
        className={`fa-solid fa-${icon} ${collapsed ? '' : 'mr-2'} text-[var(--primary-color)]`}
      ></i>
      {!collapsed && <span>{label}</span>}
      {!collapsed && status && (
        <span className={`h-2 w-2 rounded-full ml-auto mr-2 bg-${status}-500`}></span>
      )}
      {!collapsed && children}
    </NavLink>
  );
  ...
  return (
    <SidebarContext.Provider value={{ collapsed }}>
      <aside
        ...
        } border-r ${collapsed ? 'w-[70px]' : 'w-[var(--sidebar-width)]'}`}
