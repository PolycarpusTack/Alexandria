...
const CardFooter = styled.div`
  ...
  background-color: ${(props) => {
    ...
    return bgColor.startsWith('#') ? `${bgColor}20` : `${bgColor.replace(/[^,]+(?=\))/, '0.12')}`; // Handling both hex and rgba values
  }};
`;
    ...
    <CardContainer hoverable={hoverable} elevation={elevation} className={className}>
      {hasHeader && (
        <CardHeader>
          ...
          {headerAction && <CardHeaderActionContainer>{headerAction}</CardHeaderActionContainer>}
        </CardHeader>
      )}
