...
export class FileUploadSecurity {
  ...
  async validateFile(
    ...
    if (buffer.length > this.options.maxFileSize) {
      errors.push(
        `File size (${buffer.length} bytes) exceeds maximum allowed (${this.options.maxFileSize} bytes)`
      );
    }
  ...
  async generateSecureStoragePath(
    ...
    // Build secure path
    const directory = path.join(this.options.storageBasePath, dateDir, userHash);
   ...
   */
  async quarantineFile(buffer: Buffer, originalFilename: string, reason: string): Promise<string> {
    ...
    const quarantinePath = path.join(this.options.quarantinePath, quarantineFilename);
