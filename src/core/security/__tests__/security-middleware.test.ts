...
describe('SecurityMiddleware', () => {
  ...
  describe('Input Validation', () => {
    ...
    describe('XSS Prevention', () => {
      ...
      it('should sanitize nested objects', async () => {
        ...
        expect(mockReq.body.user.bio).toBe('&lt;img src=&quot;x&quot; onerror=&quot;alert(1)&quot;&gt;');
        expect(mockNext).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('Error Handling', () => {
    it('should fail open for general rate limiting errors', async () => {
      ...
      (securityMiddleware as any).aegisRateLimiter.isAllowed = jest.fn().mockRejectedValue(new Error('Test error'));
      ...
      expect(mockNext).toHaveBeenCalled();
      expect(mockLogger.error).toHaveBeenCalledWith('Rate limiter error', { error: expect.any(Error) });
