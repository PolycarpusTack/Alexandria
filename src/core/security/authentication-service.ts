...
import { AuthenticationService, Credentials, AuthResult, TokenPayload } from './interfaces';
...
export class JwtAuthenticationService implements AuthenticationService {
  ...
  constructor(
    ...
    // Critical: Enforce JWT_SECRET environment variable in production
    if (process.env.NODE_ENV === 'production' && !process.env.JWT_SECRET) {
      throw new Error(
        'JWT_SECRET environment variable is required in production. Set a strong, random secret.'
      );
    }

    if (options.jwtSecret.length < this.MIN_JWT_SECRET_LENGTH) {
      throw new Error(
        `JWT secret must be at least ${this.MIN_JWT_SECRET_LENGTH} characters long for security`
      );
    }
    ...
    if (weakSecrets.includes(options.jwtSecret.toLowerCase())) {
      throw new Error(
        'JWT secret appears to be a weak or default value. Please use a strong, random secret'
      );
    }

    // Critical: Additional production validation
    if (process.env.NODE_ENV === 'production') {
      // Ensure minimum entropy requirements for production
      if (options.jwtSecret.length < 64) {
        throw new Error(
          'JWT secret must be at least 64 characters long in production for enhanced security'
        );
      }
      
      // Validate that it's not a simple pattern
      if (/^(..)\1+$/.test(options.jwtSecret) || /^(.)\1+$/.test(options.jwtSecret)) {
        throw new Error(
          'JWT secret cannot be a repeated pattern. Use a cryptographically secure random string.'
        );
      }
    }
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      const complexityScore = [hasNumbers, hasLowerCase, hasUpperCase, hasSpecialChars].filter(
        Boolean
      ).length;

      if (complexityScore < 3) {
        this.logger.warn(
          'JWT secret has low complexity. Consider using a more complex secret with mixed character types',
          {
  ...
  async initialize(): Promise<void> {
    ...
    // Setup cleanup task for expired refresh tokens with proper cleanup reference
    this.cleanupInterval = setInterval(() => this.cleanupExpiredRefreshTokens(), 15 * 60 * 1000); // Run every 15 minutes
  ...
  async authenticate(credentials: Credentials): Promise<AuthResult> {
    ...
    const passwordValid = await this.comparePassword(password, passwordHash);
   ...
   * CRITICAL: This prevents memory leaks from the cleanup interval
   */
  destroy(): void {
    if (this.cleanupInterval) {
      clearInterval(this.cleanupInterval);
      this.cleanupInterval = undefined;
      this.logger.info('Authentication service cleanup interval destroyed', {
        component: 'JwtAuthenticationService'
      });
    }

    // Clear refresh tokens to free memory
    this.refreshTokens.clear();

    this.isInitialized = false;
    this.logger.info('Authentication service destroyed', {
      component: 'JwtAuthenticationService'
    });
  }
