...
import { QueryOptions, QueryResult, EntityType, Entity, Repository } from './pg-interfaces';
...
export class PostgresDataService implements DataService, PgDataServiceInterface {
    ...
    try {
      ...
      if (this.options.runMigrations !== false) {
        const migrationsPath =
          this.options.migrationsPath || path.join(__dirname, 'migrations', 'migrations');
      ...
      this.logger.error(
        `Failed to initialize PostgreSQL data service: ${error instanceof Error ? error.message : String(error)}`,
        {
    ...
    return {
      ...
      fields:
        result.rows.length > 0
          ? Object.keys(result.rows[0] as any).map((name) => ({
              name,
  ...
  async findOne<T extends Entity>(
    ...
    const results = await this.find<T>(entityType, criteria, {
      limit: 1,
      select: options?.select,
      useTransaction: options?.useTransaction,
      existingTransaction: options?.existingTransaction
