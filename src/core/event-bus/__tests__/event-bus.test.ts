...
describe('EventBus', () => {
  ...
  describe('subscribe', () => {
    ...
    it('should throw error with invalid parameters', () => {
      ...
      expect(() => eventBus.subscribe('test.topic', null as any)).toThrow(
  ...
  describe('subscribePattern', () => {
    ...
    it('should throw error with invalid parameters', () => {
      ...
      expect(() => eventBus.subscribePattern('test.*', null as any)).toThrow(
        'Handler must be a function'
      );
    });
  ...
  describe('Performance and Concurrency', () => {
    ...
    it('should handle concurrent publishes', async () => {
      ...
      const promises = Array(50)
        .fill(null)
        .map((_, i) => eventBus.publish('test.topic', { value: i }));
    ...
    it('should handle many different topics', async () => {
      const topics = Array(50)
        .fill(null)
        .map((_, i) => `test.topic.${i}`);
      ...
      const promises = topics.map((topic) => eventBus.publish(topic, { value: topic }));
  ...
  describe('Wildcard Patterns', () => {
    it('should support complex wildcard patterns', async () => {
      ...
      expect(handler).toHaveBeenCalledTimes(2);
      expect(handler).toHaveBeenCalledWith(expect.objectContaining({ topic: 'system.auth.error' }));
      expect(handler).toHaveBeenCalledWith(expect.objectContaining({ topic: 'system.db.error' }));
    });
