...
describe('SessionRepository', () => {
  ...
  function mockFileSystem() {
    ...
    (fs.promises.readdir as jest.Mock).mockResolvedValue(['session-1.json', 'artifact-1.json']);
  ...
  describe('initialization', () => {
    it('should create data directories if they do not exist', async () => {
      ...
      expect(fs.promises.mkdir).toHaveBeenCalledWith(path.join(mockConfig.dataPath, 'sessions'), {
        recursive: true
      });
      expect(fs.promises.mkdir).toHaveBeenCalledWith(path.join(mockConfig.dataPath, 'artifacts'), {
        recursive: true
      });
    });
  });
  ...
  describe('addSessionArtifact', () => {
    it('should add an artifact to a session', async () => {
      ...
      const artifact = await repository.addSessionArtifact('session-1', artifactData);
    ...
    it('should handle errors during artifact creation', async () => {
      ...
      await expect(
        repository.addSessionArtifact('session-1', { type: 'code_snippet', content: 'test' })
  ...
  describe('getSessionArtifacts', () => {
    it('should retrieve all artifacts for a session', async () => {
      ...
      (fs.promises.readdir as jest.Mock).mockResolvedValue(['artifact-1.json', 'artifact-2.json']);
  ...
  describe('getAllSessions', () => {
    it('should retrieve all sessions', async () => {
      ...
      (fs.promises.readdir as jest.Mock).mockResolvedValue(['session-1.json', 'session-2.json']);
  ...
  describe('deleteSession', () => {
    it('should delete a session and its artifacts', async () => {
      ...
      (repository as any).getSessionArtifacts = jest
        .fn()
        .mockResolvedValue([{ id: 'artifact-1' }, { id: 'artifact-2' }]);
      ...
      expect(fs.promises.unlink).toHaveBeenCalledWith(expect.stringContaining('session-1'));

      // Should delete artifact files
      expect(fs.promises.unlink).toHaveBeenCalledWith(expect.stringContaining('artifact-1'));
      expect(fs.promises.unlink).toHaveBeenCalledWith(expect.stringContaining('artifact-2'));
    });
    ...
    it('should handle errors during deletion', async () => {
      ...
      await expect(repository.deleteSession('session-1')).rejects.toThrow(
