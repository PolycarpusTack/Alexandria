...
export class LLMAnalysisService {
  ...
  private createLlmError(response: Response, model: string): Error {
    if (response.status === 404) {
      return new Error(`Model '${model}' not found. Please ensure Ollama is running and the model is available.`);
    } else if (response.status === 500) {
      return new Error('LLM server error. Check the server logs for details.');
    } else if (response.status === 0) {
      return new Error(`Unable to connect to LLM service. Make sure the service is running on ${process.env.OLLAMA_BASE_URL || 'http://localhost:11434'}.`);
    } else {
  ...
  private extractJsonFromResponse(responseText: string): string | null {
    ...
    const strategies = [
      ...
      () => {
        const match = responseText.match(/"(?:primaryIssue|primaryError|potentialIssues|summary)"\s*:/);
