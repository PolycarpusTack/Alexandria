...
export class EnhancedCrashAnalyzerService implements ICrashAnalyzerService {
  ...
  private async callLlmApi(
    ...
    try {
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        throw new Error(`Unable to connect to Ollama service: ${error.message}. Make sure the service is running on ${process.env.OLLAMA_BASE_URL || 'http://localhost:11434'}.`);
  ...
  async processFileUpload(
    ...
    try {
      ...
      const validationResult = await this.fileValidator.validate(buffer, sanitizedFileName, mimeType);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          throw new Error(`Unable to connect to Ollama service. Make sure the service is running on ${process.env.OLLAMA_BASE_URL || 'http://localhost:11434'}.`);
        } else {
