...
export class FileValidator {
  ...
  private readonly DEFAULT_ALLOWED_EXTENSIONS = [
    ...
    '.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.cpp', '.h', '.cs'
  ];
  ...
  constructor(
    ...
    this.options = {
      ...
      allowedCharactersInFileName: options?.allowedCharactersInFileName || this.DEFAULT_ALLOWED_CHARS
    };
  }
    ...
    try {
      ...
      if (this.isTextFile(mimeType, extension)) {
        ...
        if (securityIssues.length > 0) {
          ...
          const severeIssues = securityIssues.filter(issue => 
            issue.startsWith('CRITICAL:') || issue.startsWith('HIGH:')
          );
        ...
        if (this.options.enableDeepContentValidation) {
          ...
          if (sensitiveData.length > 0) {
            result.warnings!.push(...sensitiveData.map(data => 
              `Possible sensitive data detected: ${data}`
            ));
          }
  ...
  isTextFile(mimeType: string, extension: string): boolean {
    ...
    const textExtensions = [
      ...
      '.js', '.py', '.ts', '.jsx', '.tsx', '.java', '.c', '.cpp', '.h', '.cs'
    ];
  ...
  detectFileTypeFromContent(content: string, extension: string): string {
    ...
    if (content.includes('ERROR') || content.includes('WARN') || 
        content.includes('Exception') || content.includes('Stack trace')) {
      return 'log';
    }
  ...
  checkForSecurityIssues(content: string): string[] {
    ...
    if (entropy > 6.0) {  // English text typically has entropy around 4.5-5.5
  ...
  checkForSensitiveData(content: string): string[] {
    ...
    for (const [index, pattern] of this.SENSITIVE_DATA_PATTERNS.entries()) {
      ...
      if (matches && matches.length > 0) {
        ...
        switch (index) {
          case 0: dataType = 'Email address'; break;
          case 1: dataType = 'Credit card number'; break;
          case 2: dataType = 'Social Security Number'; break;
          case 3: dataType = 'API key'; break;
          case 4: dataType = 'IP address'; break;
          case 5: dataType = 'AWS key'; break;
          case 6: dataType = 'Password'; break;
          case 7: dataType = 'Authentication token'; break;
        }
   ...
   */
  async deepSecurityScan(buffer: Buffer, filename: string, mimeType: string): Promise<{
    isMalicious: boolean;
    ...
    try {
      ...
      if (this.isTextFile(mimeType, path.extname(filename).toLowerCase())) {
        ...
        const entropy = this.calculateEntropy(content);
        if (entropy > 7.0) { // Higher threshold than general check
          detectedThreats.push(`Extremely high entropy (${entropy.toFixed(2)}), possibly encrypted malicious content`);
          riskLevel = Math.max(riskLevel === 'low' ? 0 : riskLevel === 'medium' ? 1 : riskLevel === 'high' ? 2 : 3, 2) as any;
        }
      }
      ...
      return {
        isMalicious: true,
        detectedThreats: ['Error during security scan - file considered suspicious as a precaution'],
        riskLevel: 'medium'
      };
    }
  }
  ...
  sanitizeFileName(filename: string): string {
    ...
    const dangerousExtensions = [
      '.exe', '.bat', '.cmd', '.ps1', '.sh', '.php', '.js', '.vbs', '.py'
    ];
