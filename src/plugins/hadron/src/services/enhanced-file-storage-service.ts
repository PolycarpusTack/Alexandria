...
export class EnhancedFileStorageService {
  ...
  private readonly DEFAULT_ALLOWED_EXTENSIONS = [
    '.txt', '.log', '.json', '.xml', '.html', '.md', '.stacktrace', '.crash',
    '.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.cpp', '.h', '.cs', '.php',
    '.rb', '.go', '.rs', '.kt', '.swift', '.dart', '.scala', '.clj', '.vue',
    '.csv', '.yaml', '.yml', '.toml', '.ini', '.conf', '.config', '.properties',
    '.dmp', '.minidump', '.core', '.backtrace', '.gdb', '.strace', '.ltrace'
  ];
  ...
  async storeFile(
    ...
    try {
      this.logger.info(`Processing file upload: ${originalFilename} for user ${userId} session ${sessionId}`);
      ...
      if (this.config.enableAutoScan) {
        ...
        if (securityScanResult.isMalicious) {
          if (securityScanResult.riskLevel === 'critical' || securityScanResult.riskLevel === 'high') {
            return {
              success: false,
            ...
            quarantined = true;
            result.warnings!.push(`File flagged for security review: ${securityScanResult.detectedThreats.join(', ')}`);
          }
        }
      }
      
      // 6. Generate storage paths
      const storageInfo = this.generateStoragePaths(userId, sessionId, originalFilename, quarantined);
      
      // 7. Store file to disk
      await this.writeFileToStorage(buffer, storageInfo.filePath, quarantined ? storageInfo.quarantinePath : undefined);
      ...
      const uploadedFile = new UploadedFile({
        ...
        metadata: {
          ...
          quarantined,
          storageInfo: {
            relativePath: path.relative(this.config.baseStoragePath, quarantined ? storageInfo.quarantinePath! : storageInfo.filePath),
   ...
   */
  async readFile(fileId: string, userId: string): Promise<{ success: boolean; content?: string; error?: string }> {
    try {
   ...
   */
  private async checkUserQuota(userId: string, additionalBytes: number): Promise<{
    allowed: boolean;
    used: number;
    available: number;
    quota: number;
   ...
   */
  private generateStoragePaths(userId: string, sessionId: string, filename: string, quarantined: boolean) {
    ...
    if (quarantined) {
      quarantinePath = path.join(this.config.quarantineDir, `${crypto.randomUUID()}_${this.fileValidator.sanitizeFileName(filename)}`);
    }
   ...
   */
  private async writeFileToStorage(buffer: Buffer, filePath: string, quarantinePath?: string): Promise<void> {
    // Create directories
  ...
  private validateFilePath(filePath: string): void {
    ...
    if (!resolvedPath.startsWith(resolvedBasePath) && !resolvedPath.startsWith(resolvedQuarantinePath)) {
