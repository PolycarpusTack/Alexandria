...
export class CentralizedAIAdapter implements ILlmService {
  ...
  async checkAvailability(): Promise<boolean> {
    try {
      ...
      this.logger.warn('AI service availability check failed:', { error: error instanceof Error ? error.message : String(error) });
      return false;
    }
  }
  ...
  async analyzeLog(
    ...
    try {
      ...
      const messages: ChatMessage[] = [
        {
          role: 'system',
          content: 'You are an expert software debugger analyzing crash logs. Provide detailed, structured analysis in JSON format.'
        },
        {
          role: 'user',
   ...
   */
  async getModelTiers(): Promise<{ [key in ModelTier]: { models: string[]; description: string; goodFor: string; maxTokens: number } }> {
  ...
  private async resolveModel(customModel?: string, analysisType?: string): Promise<string> {
    ...
    const chatModels = models.filter(m => 
      m.capabilities.includes(AICapability.CHAT) || 
      m.capabilities.includes(AICapability.INSTRUCT)
    );
  ...
  private async parseAnalysisResponse(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn('Error parsing AI response JSON, attempting to fix:', { error: String(jsonError) });
