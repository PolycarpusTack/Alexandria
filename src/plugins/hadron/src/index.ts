...
export default class CrashAnalyzerPlugin implements PluginLifecycle {
  ...
  async install(context: PluginContext): Promise<void> {
    ...
    const storageOptions = {
      baseStoragePath:
        process.env.HADRON_STORAGE_PATH || process.env.STORAGE_PATH || './storage/hadron',
      maxSizeBytes: parseInt(process.env.HADRON_MAX_FILE_SIZE || '20971520'), // 20MB default
      allowedExtensions: (
        process.env.HADRON_ALLOWED_EXTENSIONS ||
        '.txt,.log,.json,.xml,.html,.md,.stacktrace,.crash,.py,.js,.ts,.jsx,.tsx,.java,.cpp,.h,.cs'
      ).split(',')
    };
    ...
    this.crashAnalyzerService = new EnhancedCrashAnalyzerService(
      ...
      context.services.logger,
      {
        baseStoragePath:
          process.env.HADRON_STORAGE_PATH || process.env.STORAGE_PATH || './storage/hadron',
        quarantineDir:
          process.env.HADRON_QUARANTINE_PATH ||
          process.env.QUARANTINE_PATH ||
          './storage/hadron/quarantine'
      }
    );
    ...
    if (this.crashRepository && this.hadronRepository) {
      ...
      this.alertManager = new AlertManager(context.services.data, context.services.eventBus);
      this.notificationService = new NotificationService(context.services.eventBus);
