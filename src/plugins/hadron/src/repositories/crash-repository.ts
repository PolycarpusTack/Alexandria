...
export class CrashRepository implements ICrashRepository {
  ...
  private getNestedValue(
    obj: INestedObject,
    path: string
  ): string | number | boolean | INestedObject | Array<any> {
  ...
  async deleteCrashLog(id: string): Promise<boolean> {
    ...
    if (analyses.length > 0) {
      ...
      const deletionResults = await Promise.allSettled(
        analyses.map((analysis) => this.dataService.delete(this.analysisCollection, analysis.id))
      );
  ...
  async getAnalysisById(id: string): Promise<CrashAnalysisResult | null> {
    return this.dataService.findById(
      ...
      id
    ) as Promise<CrashAnalysisResult | null>;
  }
  ...
  async getAnalysesByCrashLogId(crashLogId: string): Promise<CrashAnalysisResult[]> {
    return this.dataService.find(this.analysisCollection, { crashLogId }) as Promise<
      CrashAnalysisResult[]
    >;
  }
}
