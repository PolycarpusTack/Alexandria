...
import { Search, MessageSquare, Trash2, Clock, FileText } from 'lucide-react';
        ...
        <div className='space-y-2'>
          {filteredSessions.length === 0 ? (
            ...
            filteredSessions.map((session) => (
              <div
                key={session.id}
                className={`alfred-session-item ${session.id === currentSessionId ? 'active' : ''}`}
              ...
              >
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <span className='text-xs text-muted-foreground'>{session.metadata.model}</span>
                </div>
              </div>
            ))
          )}
