        ...
        <Tooltip.Content>
          <div className='text-sm'>
            ...
            {connectionInfo.status === 'connected' && (
              <>
                <div>
                  {connectionInfo.provider} - {connectionInfo.model}
                </div>
                <div>Latency: {formatLatency(connectionInfo.latency)}</div>
              </>
            )}
            {connectionInfo.error && <div className='text-red-500'>{connectionInfo.error}</div>}
          </div>
        </Tooltip.Content>
      </Tooltip>
    );
  ...
  return (
    <div className={`alfred-connection-status ${className}`}>
      ...
      {connectionInfo.status === 'error' && connectionInfo.error && (
        <div className='mt-2 text-sm text-red-500'>{connectionInfo.error}</div>
      )}
    </div>
  );
};
