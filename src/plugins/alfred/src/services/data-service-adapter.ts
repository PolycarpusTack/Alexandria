...
import { PostgresCollectionAdapter, CollectionDataService } from '../../../../core/data/collection-adapter';
...
export class AlfredDataServiceAdapter {
   ...
   */
  static async getInstance(dataService: DataService, logger: Logger): Promise<CollectionDataService> {
    if (!this.instance) {
      // Check if the data service is PostgreSQL-based
      if (dataService instanceof PostgresDataService || 'query' in dataService) {
        this.instance = new PostgresCollectionAdapter(
          dataService as PostgresDataService,
          logger
        );
   ...
   * Initialize Alfred-specific collections
   */
  private static async initializeCollections(collectionService: CollectionDataService): Promise<void> {
