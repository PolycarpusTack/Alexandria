...
export class HadronRepository {
   ...
   */
  async findFilesByMetadataField(field: string, value: any): Promise<UploadedFile[]> {
    try {
   ...
   */
  private toDbRecord(obj: any): Record<string, any> {
    // Input validation
    ...
    try {
        ...
        if (key === 'metadata' || key === 'preferences' || key === 'tags' || 
          ...
          if (value !== null && (Array.isArray(value) || typeof value === 'object' || 
            ...
            try {
              ...
              console.error(`Error serializing field ${key} to JSON:`, jsonError);
      ...
      console.error('Error converting object to database record:', error);
