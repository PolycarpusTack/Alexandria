...
export class EnhancedLlmService implements ILlmService {
  ...
  async checkAvailability(): Promise<boolean> {
    try {
      ...
      this.logger.warn('Ollama availability check failed:', { error: error instanceof Error ? error.message : String(error) });
      return false;
    }
  }
  ...
  async analyzeLog(
    ...
    try {
      ...
      // Generate the enhanced prompt with context-aware information
      const prompt = await this.generateEnhancedPrompt(parsedData, rawContent);
      
      // Call the LLM with retries and improved error handling
      const response = await this.callLlmWithRetry(model, prompt);
      ...
      this.logger.error('Error analyzing log with LLM:', { error: error instanceof Error ? error.message : String(error) });
  ...
  async getAvailableModels(forceRefresh: boolean = false): Promise<string[]> {
    ...
    try {
      ...
      this.logger.error('Error fetching available models:', { error: error instanceof Error ? error.message : String(error) });
  ...
  async getModelStatus(modelId: string): Promise<ModelStatus> {
    try {
      ...
      this.logger.error(`Error getting status for model ${modelId}:`, { error: error instanceof Error ? error.message : String(error) });
  ...
  async recommendModel(
    ...
    if (complexity > 0.8) {
      ...
      if (xlModels.length > 0) {
        return {
          modelId: xlModels[0],
          reason: 'Complex crash with multiple error patterns and stack traces. XL model recommended for most accurate analysis.',
          tier: 'xl'
        };
      } else if (largeModels.length > 0) {
        return {
          modelId: largeModels[0],
          reason: 'Complex crash with multiple error patterns and stack traces. Large model recommended for accurate analysis.',
          tier: 'large'
        };
      }
      ...
      if (largeModels.length > 0) {
        return {
          modelId: largeModels[0],
          reason: 'Moderately complex crash with multiple error indicators. Large model recommended for good analysis quality.',
          tier: 'large'
        };
      ...
      if (mediumModels.length > 0) {
        return {
          modelId: mediumModels[0],
          reason: 'Simple crash with clear error patterns. Medium model will provide good results with efficient resource usage.',
          tier: 'medium'
        };
      } else if (smallModels.length > 0) {
        return {
          modelId: smallModels[0],
          reason: 'Simple crash. Small model should be sufficient for basic analysis with minimal resource usage.',
          tier: 'small'
        };
      }
    }
  ...
  private async generateEnhancedPrompt(
    ...
    const stackTraces = parsedData.stackTraces.map(st => {
      const frames = st.frames.map((frame, index) => {
...
Ensure your analysis is technically precise, focusing on the evidence provided in the logs.`;
  ...
  /**
   * Call the LLM with improved error handling and retries
   * 
   * @param model Model to use
   * @param prompt Prompt text
   * @returns LLM response
   */
  private async callLlmWithRetry(model: string, prompt: string): Promise<any> {
    let attempts = 0;
    let lastError: Error | null = null;
    
    while (attempts <= this.MAX_RETRIES) {
      try {
        this.logger.debug(`Attempt ${attempts + 1} to call Ollama with model ${model}`);
        ...
        const response = await fetch(`${this.OLLAMA_BASE_URL}/generate`, {
          ...
          body: JSON.stringify({
            ...
            options: {
              ...
              num_predict: 4096, // Allow longer responses for complex analyses
  ...
  private async parseAnalysisResponse(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn('Error parsing LLM response JSON, attempting to fix:', { error: String(jsonError) });
  ...
  private async resolveModel(customModel?: string): Promise<string> {
    try {
      ...
      if (modelFromFlag && typeof modelFromFlag === 'string' && availableModels.includes(modelFromFlag)) {
      ...
      this.logger.error('Error determining model to use:', { error: error instanceof Error ? error.message : String(error) });
      return this.FALLBACK_MODEL;
    }
  }
...
Focus on the most important issues first, and provide actionable feedback.`;
  ...
  private async parseCodeAnalysisResponse(
    ...
    try {
      ...
      if (!analysisData.potentialIssues || !Array.isArray(analysisData.potentialIssues) || analysisData.potentialIssues.length === 0) {
      ...
      if (!analysisData.improvementSuggestions || !Array.isArray(analysisData.improvementSuggestions)) {
        analysisData.improvementSuggestions = ['Consider adding comments to explain the code logic'];
      }
  ...
  private async resolveModelForCodeAnalysis(code: string, language: string): Promise<string> {
    ...
    try {
      const customModel = await this.featureFlagService.getValue('crash-analyzer.code-analysis.model');
  ...
  private analyzeCodeComplexity(code: string, language: string): number {
    ...
    const maxIndent = indentMatches ? 
      Math.max(...indentMatches.map(m => m.length)) : 0;
    ...
    const functionPattern = language.toLowerCase().includes('java') || 
  ...
  private analyzeCrashComplexity(parsedData: ParsedCrashData, rawContent: string): number {
    ...
    const stackDepth = parsedData.stackTraces.reduce((max, st) => 
      Math.max(max, st.frames.length), 0);
    ...
    if (rawContent.length > 100000) { // 100KB
      complexityScore += 0.15;
    } else if (rawContent.length > 10000) { // 10KB
    ...
    if (rawContent.includes('thread') || rawContent.includes('concurrent') || 
        rawContent.includes('parallel') || rawContent.includes('deadlock') ||
        rawContent.includes('synchroniz')) {
      complexityScore += 0.1;
    }
    
    // Check for memory issues
    if (rawContent.includes('memory') || rawContent.includes('leak') ||
        rawContent.includes('allocation') || rawContent.includes('heap') ||
