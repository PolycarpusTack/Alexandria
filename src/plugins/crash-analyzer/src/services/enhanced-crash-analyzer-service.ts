...
export class EnhancedCrashAnalyzerService implements ICrashAnalyzerService {
  ...
  private async callLlmApi(
    ...
    try {
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        throw new Error(`Unable to connect to Ollama service: ${error.message}. Make sure the service is running on localhost:11434.`);
      } else if (error.name === 'AbortError') {
        throw new Error(`Request to Ollama service timed out after ${timeoutMs/1000} seconds. The model may be overloaded or unavailable.`);
      }
  ...
  async createSession(
    ...
    try {
      ...
      this.logger.error('Error creating analysis session:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async processFileUpload(
    ...
    try {
      ...
      this.logger.error('Error processing file upload:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async saveCodeSnippet(
    ...
    try {
      ...
      this.logger.error('Error saving code snippet:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(`Model '${model}' not found. Please ensure Ollama is running and the model is available.`);
        } else if (response.status === 500) {
          throw new Error(`Ollama server error. Check the server logs for details.`);
        } else if (response.status === 0) {
          throw new Error(`Unable to connect to Ollama service. Make sure the service is running on localhost:11434.`);
        } else {
      ...
      if (!extractedJson) {
        ...
        throw new Error('No valid JSON data found in LLM response. The model may need to be prompted differently.');
      }
      ...
      try {
        ...
        try {
          ...
          throw new Error('Invalid JSON format in LLM response. The response could not be parsed correctly. Original error: ' + 
      ...
      if (!analysisData.primaryIssue || !analysisData.potentialIssues || !analysisData.summary) {
        ...
        throw new Error('LLM response is missing required fields. The model may need to be prompted differently.');
      }
      ...
      this.logger.error(`Error analyzing code snippet: ${snippetId}`, { error: error instanceof Error ? error.message : String(error) });
  ...
  /**
   * Get crash logs with enhanced filtering
   * 
  ...
  async getFilteredCrashLogs(options: {
    ...
    try {
      // Start with all logs
      let logs = await this.crashRepository.getAllCrashLogs();
      ...
      if (options.sessionId) {
        logs = logs.filter(log => log.metadata?.sessionId === options.sessionId);
      }
      
      if (options.startDate && options.startDate instanceof Date) {
        logs = logs.filter(log => log.uploadedAt >= options.startDate!);
      }
      
      if (options.endDate && options.endDate instanceof Date) {
        logs = logs.filter(log => log.uploadedAt <= options.endDate!);
      }
      ...
      if (options.searchTerm) {
        ...
        logs = logs.filter(log => 
          ...
          log.analysis?.summary.toLowerCase().includes(term)
        );
      }
      ...
      this.logger.error('Error getting filtered crash logs:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  private generateLogTitle(content: string, metadata: any): string {
    ...
    } else if (content.includes('exception') && (content.includes('java.') || content.includes('android.'))) {
      logType = 'Java Exception';
    } else if (content.includes('EXC_BAD_ACCESS') || content.includes('SIGABRT')) {
      logType = 'iOS Crash';
    } else if (content.includes('JavaScript') || content.includes('TypeError') || content.includes('ReferenceError')) {
  ...
  private async getAvailableModels(): Promise<string[]> {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new Error(`Request to Ollama service timed out after ${timeoutMs/1000} seconds when fetching available models.`);
      }
      throw new Error(`Error getting available models: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      // Always clear the timeout to prevent memory leaks
      clearTimeout(timeout);
    }
