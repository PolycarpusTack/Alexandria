...
export class EnhancedLogParser implements ILogParser {
  ...
  private errorMessageRegex = /((?:ERROR|FATAL|EXCEPTION|FAILURE|CRASH|ASSERTION|PANIC)(?:\s+|:))(.*?)($|\n)/gm;
  ...
  private iosStackFrameRegex = /\d+\s+([a-zA-Z0-9_]+)\s+(0x[0-9a-f]+)\s+([a-zA-Z0-9_]+)\s+\+\s+(\d+)/gm;
  ...
  private systemInfoRegex = {
    ...
    osVersion: /(?:OS|Operating System)(?:\s+Version)?[\s:]+\s*([^\n\r,;]+)|(?:Android|iOS|Windows|macOS|Linux)\s+([\d\.]+)/i,
  ...
  supportsFormat(content: string, metadata?: any): boolean {
    ...
    if (metadata?.filename) {
      ...
      const supportedExtensions = [
        '.log', '.stacktrace', '.crash', '.txt', '.json', '.xml', '.html'
      ];
  ...
  private detectLogType(content: string, metadata?: any): string {
    ...
    if (content.startsWith('{') && content.includes('"') && (content.includes('":"') || content.includes('": "')) && content.includes('}')) {
      return 'json';
    }
    
    if (content.startsWith('<') && (content.includes('<?xml') || content.match(/<.*>[\s\S]*<\/.*>/) !== null)) {
      return 'xml';
    }
    
    if (content.match(this.androidExceptionRegex) !== null && 
        content.match(/(?:android|com\.android|java\.lang)/) !== null) {
      return 'android';
    }
    
    if (content.match(this.iosExceptionRegex) !== null &&
        (content.includes('iOS') || content.includes('iPhone') || content.includes('iPad') || 
         content.includes('SIGABRT') || content.includes('libc++'))) {
      return 'ios';
    }
    
    if (content.match(this.jsErrorRegex) !== null &&
        (content.includes('Chrome') || content.includes('Firefox') || content.includes('Safari') ||
         content.includes('ReferenceError') || content.includes('node:') || content.includes('webpack'))) {
      return 'javascript';
    }
    
    if (content.match(this.pythonTracebackRegex) !== null || 
  ...
  private parseAndroidLog(content: string, parsedData: ParsedCrashData): void {
    ...
    if (manufacturerMatch && manufacturerMatch[1]) {
      ...
      parsedData.systemInfo.deviceModel = manufacturerMatch[1].trim() + ' ' + parsedData.systemInfo.deviceModel;
    }
  }
  ...
  private parseIOSLog(content: string, parsedData: ParsedCrashData): void {
    ...
    if (frames.length > 0) {
      ...
      const crashReasonRegex = /Exception (?:Type|Subtype|Codes|Message|Reason):[^\n]+(\w+\s+[^\n]+)/i;
  ...
  private parseJSONLog(content: string, parsedData: ParsedCrashData): void {
    try {
      ...
      if (jsonData.error || jsonData.exception || jsonData.stacktrace || jsonData.message) {
        ...
        const errorMessage = jsonData.error?.message || jsonData.exception?.message || 
                             jsonData.message || jsonData.error || jsonData.exception;
        ...
        // Extract stack trace
        const stacktrace = jsonData.stacktrace || jsonData.stack || 
                           jsonData.error?.stacktrace || jsonData.exception?.stacktrace;
      ...
      if (jsonData.system || jsonData.device || jsonData.context || jsonData.systemInfo) {
        const sysInfo = jsonData.system || jsonData.device || jsonData.context || jsonData.systemInfo;
          ...
          for (const [srcKey, destKey] of Object.entries(mapping)) {
            if (srcKey in sysInfo && sysInfo[srcKey] !== undefined && sysInfo[srcKey] !== null) {
              ...
              if (destKey === 'otherHardwareInfo' || destKey === 'otherSoftwareInfo') {
                ...
                } else if (destKey === 'otherSoftwareInfo' && parsedData.systemInfo.otherSoftwareInfo) {
                  parsedData.systemInfo.otherSoftwareInfo[srcKey] = sysInfo[srcKey];
                }
              } else {
                // For specified string properties
                parsedData.systemInfo[destKey as keyof Omit<SystemInfo, 'otherHardwareInfo' | 'otherSoftwareInfo'>] = String(sysInfo[srcKey]);
              }
            }
          }
        }
  ...
  private parseXMLLog(content: string, parsedData: ParsedCrashData): void {
    ...
    const exceptionTagRegex = /<exception(?:\s+[^>]*)?>[\s\S]*?<message(?:\s+[^>]*)?>([^<]*)<\/message>[\s\S]*?<\/exception>/gi;
    ...
    if (systemMatch) {
      ...
      const appVersionMatch = /<app_version(?:\s+[^>]*)?>([^<]*)<\/app_version>/i.exec(systemContent);
  ...
  private async enhanceWithChunker(content: string, parsedData: ParsedCrashData): Promise<void> {
    ...
    try {
      ...
      if (this.logger) {
        ...
        console.error('Error enhancing with chunker:', error);
      }
