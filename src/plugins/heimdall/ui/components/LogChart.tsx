...
export const LogChart: React.FC<LogChartProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    const points: ChartDataPoint[] = data.map((item, index) => {
      ...
      const timestamp = item.key_as_string || item.timestamp || item.time || new Date(Date.now() - (data.length - index) * 60000);
    ...
    return points.map((point, index) => {
      ...
      if (config.algorithm === 'ml' || config.algorithm === 'hybrid') {
        ...
        if (deviation > config.sensitivity) {
          isAnomaly = true;
          severity = deviation > 0.8 ? 'critical' : deviation > 0.6 ? 'high' : deviation > 0.4 ? 'medium' : 'low';
        }
      }
  ...
  const avgValue = chartData.length > 0 ? chartData.reduce((sum, d) => sum + d.value, 0) / chartData.length : 0;
  const anomalies = chartData.filter(d => d.isAnomaly);
  const trendDirection = chartData.length > 1 ? 
    (chartData[chartData.length - 1].value > chartData[0].value ? 'up' : 
     chartData[chartData.length - 1].value < chartData[0].value ? 'down' : 'stable') : 'stable';
  ...
  const renderLineChart = () => (
    <div className="relative" style={{ height: `${height}px` }}>
      <svg width="100%" height="100%" className="overflow-visible">
        ...
        {/* Anomaly regions */}
        {showAnomaly && anomalies.map((anomaly, index) => {
          const dataIndex = chartData.findIndex(d => d.time.getTime() === anomaly.time.getTime());
        ...
        <g>
          {chartData.filter((_, index) => index % Math.ceil(chartData.length / 6) === 0).map((point, index) => {
            const dataIndex = chartData.findIndex(d => d.time.getTime() === point.time.getTime());
            const x = 40 + (dataIndex / (chartData.length - 1)) * 55;
      ...
      <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: any) => setAnomalyConfig(prev => ({ ...prev, algorithm: value }))}
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setAnomalyConfig(prev => ({ ...prev, sensitivity: Number(e.target.value) }))}
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="text-xs text-muted-foreground">
            ...
            {selectedRange && (
              <span> • Zoomed: {selectedRange.start}-{selectedRange.end}</span>
            )}
          </div>
        </div>
      </CardContent>
