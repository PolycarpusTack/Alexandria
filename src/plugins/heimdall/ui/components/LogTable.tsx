...
export const LogTable: React.FC<LogTableProps> = ({
  ...
  const filteredAndSortedLogs = useMemo(() => {
    ...
    if (filters.source !== 'all') {
      processed = processed.filter(
        (log) => log.source?.service === filters.source || log.source?.host === filters.source
      );
    }
  ...
  const createAlertFromLog = useCallback(
    async (log: any) => {
      if (onCreateAlert) {
        onCreateAlert(log);
      } else {
  ...
  const formatCellValue = useCallback(
    (log: HeimdallLogEntry, column: TableColumn) => {
                ...
                {showActions && (
                  <th className='px-4 py-3 text-left w-12'>
                    <input
                      type='checkbox'
                      checked={
                        selectedRows.size === paginatedLogs.length && paginatedLogs.length > 0
                      }
                      onChange={() =>
                          ...
                          <div className='space-y-4'>
                            ...
                            <div className='flex gap-2 pt-2 border-t'>
                              ...
                              <Button
                                variant='outline'
                                size='sm'
                                onClick={() => analyzeCorrelations(log)}
                              >
                              ...
                              <Button
                                variant='outline'
                                size='sm'
                                onClick={() => createAlertFromLog(log)}
                              >
