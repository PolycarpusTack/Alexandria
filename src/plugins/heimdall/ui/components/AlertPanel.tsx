...
export const AlertPanel: React.FC<AlertPanelProps> = ({
  ...
  const toggleAlert = async (alertId: string, enabled: boolean) => {
    try {
      ...
      setAlerts(alerts.map((alert) => (alert.id === alertId ? { ...alert, enabled } : alert)));
    } catch (err) {
  ...
  const getAlertSeverityColor = (alert: LogAlert): string => {
    ...
    if (
      condition.query?.levels?.includes('error' as any) ||
  ...
  return (
    <Card className='p-4'>
      <div className='flex justify-between items-center mb-4'>
        ...
        <Button onClick={() => setShowCreateForm(true)} size='small'>
          Create Alert
        </Button>
      </div>
      ...
      {alerts.length === 0 ? (
        <div className='text-center py-8 text-gray-500'>
          ...
          <Button onClick={() => setShowCreateForm(true)} variant='outline' className='mt-2'>
        ...
        <div className='space-y-3'>
          {alerts.map((alert) => (
            <div key={alert.id} className={`border rounded-lg p-4 ${getAlertSeverityColor(alert)}`}>
              <div className='flex justify-between items-start'>
                <div className='flex-1'>
                  ...
                  <p className='text-sm text-gray-600 mt-1'>{getConditionSummary(alert)}</p>
