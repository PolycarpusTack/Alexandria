...
import Router, { Router, Request, Response, NextFunction } from 'express';
...
export class HeimdallAPI {
  ...
  private async handleListQueries(req: Request, res: Response): Promise<void> {
    try {
      ...
      const queries = await this.context
        .getDataService()
        .query(
  ...
  private async handleGetQuery(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const query = await this.context.getDataService().findOne('heimdall_queries', { id });
  ...
  private async handleUpdateQuery(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updatedQuery = await this.context
        .getDataService()
        .update('heimdall_queries', { id }, updates);
  ...
  private async handleListAlerts(req: Request, res: Response): Promise<void> {
    try {
      ...
      const alerts = await this.context
        .getDataService()
        .query('SELECT * FROM heimdall_alerts ORDER BY created_at DESC');
  ...
  private async handleGetAlert(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const alert = await this.context.getDataService().findOne('heimdall_alerts', { id });
  ...
  private async handleUpdateAlert(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updatedAlert = await this.context
        .getDataService()
        .update('heimdall_alerts', { id }, updates);
   ...
   * Permission checking
   */
  private async checkPermission(req: Request, action: string, resource?: any): Promise<void> {
