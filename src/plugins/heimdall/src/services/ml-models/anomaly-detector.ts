...
export class AnomalyDetector {
    ...
    this.context = {
      baselineStats: {
        ...
        serviceBaselines: new Map([
          ['auth', { avgResponseTime: 150, errorThreshold: 0.01, normalPatterns: ['login', 'logout', 'token'] }],
          ['api', { avgResponseTime: 200, errorThreshold: 0.03, normalPatterns: ['GET', 'POST', 'PUT'] }],
          ['worker', { avgResponseTime: 500, errorThreshold: 0.05, normalPatterns: ['process', 'queue', 'complete'] }]
        ])
      },
      recentWindow: []
    };
  ...
  async detectAnomaly(log: HeimdallLogEntry): Promise<number> {
    ...
    const weightedScore = anomalyScores.reduce((sum, score, index) => 
  ...
  private detectPatternAnomaly(log: HeimdallLogEntry): number {
    ...
    const suspiciousPatterns = [
      'exception', 'error', 'failed', 'timeout', 'refused',
      'denied', 'unauthorized', 'forbidden', 'crash', 'panic'
    ];
  ...
  private detectVolumeAnomaly(log: HeimdallLogEntry): number {
    ...
    const recentAvg = this.context.recentWindow
      .slice(-10)
      .reduce((sum, window) => sum + window.count, 0) / 10;
  ...
  private detectServiceAnomaly(log: HeimdallLogEntry): number {
    ...
    const serviceHistory = this.context.recentWindow
      .flatMap(window => Array.from(window.services.keys()));
  ...
  private detectCollectiveAnomalies(logs: HeimdallLogEntry[]): Map<string, number> {
    ...
    logs.forEach(log => {
      if (log.level === LogLevel.ERROR || log.level === LogLevel.FATAL) {
        serviceErrors.set(
          log.source.service,
          (serviceErrors.get(log.source.service) || 0) + 1
        );
      }
    });
  ...
  private updateContext(logs: HeimdallLogEntry[]): void {
    ...
    minuteGroups.forEach((groupLogs, minute) => {
      ...
      groupLogs.forEach(log => {
        window.services.set(
          log.source.service,
          (window.services.get(log.source.service) || 0) + 1
        );
      });
  ...
  private calculateConfidence(anomalyScore: number): number {
    ...
    if (anomalyScore > 0.8) return 0.90;
